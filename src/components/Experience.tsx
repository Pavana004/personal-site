      ...
      <div className="container px-6">
        ...
        >
          ...
          <p className="text-muted-foreground mt-4 max-w-2xl mx-auto">
            3+ years of building impactful products in fast-paced startup environments.
          </p>
        </motion.div>
        ...
        <div className="max-w-4xl mx-auto relative">
          ...
          {experiences.map((exp, index) => (
            ...
            >
              {/* Timeline dot */}
              <div className="absolute left-0 md:left-auto md:-translate-x-1/2 top-0 w-4 h-4 rounded-full bg-gradient-primary glow-sm hidden md:block" 
                   style={{ [index % 2 === 0 ? 'right' : 'left']: '-2rem', transform: 'translateX(50%)' }} />
              ...
              <div className="p-6 rounded-xl glass-card hover:shadow-lg transition-shadow ml-8 md:ml-0">
                ...
                <ul className={`space-y-2 ${index % 2 === 0 ? "md:text-right" : ""}`}>
                  {exp.achievements.map((achievement, i) => (
                    <li key={i} className={`text-sm flex items-start gap-2 ${index % 2 === 0 ? "md:flex-row-reverse" : ""}`}>
